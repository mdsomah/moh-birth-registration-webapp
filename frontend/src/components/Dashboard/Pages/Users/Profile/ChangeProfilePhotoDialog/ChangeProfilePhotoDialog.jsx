...
//? Endpoints
const postUserProfilePhotoURL = "/users/profile/photo";
...
// Validate User Profile Photo Schema
const ValidateUserProfilePhotoSchema = Yup.object()
  .shape({
    photo: Yup.mixed()
      .notRequired()
      .test(
...
const ChangeProfilePhotoDialog = ({ open, handleClose }) => {
  ...
  //? Formik User Profile Photo Form
  const formikUserProfilePhotoForm = useFormik({
    initialValues: {
      photo: "",
    },
    validationSchema: ValidateUserProfilePhotoSchema,
    onSubmit: () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    formikUserProfilePhotoForm.handleSubmit();
  };

  //? Handle Cancel
  const handleCancel = () => {
    handleClose();
    formikUserProfilePhotoForm.resetForm();
  };

  const onDrop = useCallback(
    (acceptedFiles) => {
      formikUserProfilePhotoForm.setFieldValue("photo", acceptedFiles[0]);
      setFiles(
  ...
  //? User Data
  const UserData = {
    photo: formikUserProfilePhotoForm.values.photo,
  };
  ...
  const Mutation = useMutation({
    mutationFn: (newData) => CreateData(`${postUserProfilePhotoURL}`, newData),
    onSuccess: (data) => {
      if (data) {
        handleClose();
        formikUserProfilePhotoForm.resetForm();
...
export default React.memo(ChangeProfilePhotoDialog);

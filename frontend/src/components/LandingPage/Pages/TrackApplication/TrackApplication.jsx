...
// Endpoints
const trackApplicationURL = "/institutions/track-application";

// Track Application Schema
const ValidateTrackApplicationSchema = Yup.object()
  .shape({
...
const TrackApplication = () => {
  ...
  // Formik Track Application Form
  const formikTrackApplicationForm = useFormik({
    initialValues: {
      businessTIN: 0,
    },
    validationSchema: ValidateTrackApplicationSchema,
    onSubmit: (values) => {
      console.log(values);
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    formikTrackApplicationForm.handleSubmit();
  };

  // Track Application Data
  const TrackApplicationData = {
    businessTIN: formikTrackApplicationForm.values.businessTIN,
  };

  console.log(TrackApplicationData);

  // useQueryClient
  const queryClient = useQueryClient();

  const Mutation = useMutation({
    mutationFn: (newData) => CreateData(`${trackApplicationURL}`, newData),
    onSuccess: (data) => {
      if (data) {
  ...
  const handleTrackApplication = () => {
    Mutation.mutate(TrackApplicationData);
  };

  // Scroll to error input on form submit
  useEffect(() => {
    if (!formikTrackApplicationForm.isSubmitting) return;
    if (Object.keys(formikTrackApplicationForm.errors).length > 0) {
      document
        .getElementsByName(Object.keys(formikTrackApplicationForm.errors)[0])
        .forEach((error) => error.focus());
    }
  }, [formikTrackApplicationForm]);
  ...
  return (
    <React.Fragment>
      <Helmet>
        <meta charSet="utf-8" />
        <title>
          Track Application | MICAT online registration web software.
        </title>
      </Helmet>
              ...
              <Box sx={{ mt: 4 }}>
                <Box sx={{ mb: 2 }}>
                  ...
                  <Typography sx={{ color: "#F5F5DC", fontSize: 16, mt: 1 }}>
                    Enter a valid email address that already exit in MICAT
                    database to track the status of your application process
                  </Typography>
                </Box>
                <Box
                  sx={{
              ...
              >
                ...
                <Typography sx={{ mt: 2 }}>
                  <span style={{ color: "#4169E1" }}>
                    Enter your business TIN to track the status of your
                    application.
                  </span>
                </Typography>
                <Box
                ...
                >
                  <FormControl variant="outlined" fullWidth>
                    ...
                    <TextField
                      ...
                      value={formikTrackApplicationForm.values.businessTIN}
                      onChange={formikTrackApplicationForm.handleChange}
                      onBlur={formikTrackApplicationForm.handleBlur}
                      error={
                        formikTrackApplicationForm.touched.businessTIN &&
                        Boolean(formikTrackApplicationForm.errors.businessTIN)
                      }
                    />
                    <Typography variant="inherit" color="error.main">
                      {formikTrackApplicationForm.touched.businessTIN &&
                        formikTrackApplicationForm.errors.businessTIN}
                    </Typography>
                  </FormControl>
                  <LoadingButton
                    type="submit"
